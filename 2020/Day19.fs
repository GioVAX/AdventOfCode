...
type Reference =
    ...
    | Alternative of ( int list * int list)
...
let addRule (map:Map<int, Rule>) r =
    match r with
    | MultiRegex @"^(?<idx>\d+): (?<s1>(\d+) ?)+ \| (?<s2>(\d+) ?)+$" matches ->
        (matches.["idx"].[0] |> int, 
         { Uses=Alternative (matches.["s1"] |> List.map int, matches.["s2"] |> List.map int);
           Regex=""})
    | MultiRegex @"(?<idx>\d+): (?<s1>(\d+) ?)+$" matches ->
        (matches.["idx"].[0] |> int,
         { Uses=Sequence (matches.["s1"] |> List.map int);
           Regex=""})
    | Regex @"^(\d+): (\d+)$" [ruleNumber; ref] ->
        (ruleNumber |> int, {Uses= ref |> int |> Single ; Regex=""})
    | Regex @"^(\d+): ""(.)""$" [ruleNumber; value] ->
        (ruleNumber |> int, {Uses=NoRef; Regex=value})
    | _ -> failwith $"Rule syntax error - <{r}>"
    |> map.Add
