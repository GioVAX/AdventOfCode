...
type Command =
    | Mask of string
    | MemoryWrite of (int * int64)

let parse = function
    | Regex @"mem\[(\d+)\] = (\d+)" [cell; value] ->
        MemoryWrite (cell |> int , value |> int64)
    | Regex @"mask = (.*)" [mask] ->
        Mask mask
    | _ -> failwith "Syntax error"

let rec convertToBinary (n:int64) =
    match n with
    | 0L 
    | 1L -> string n
    | _ ->
        let bit = string (n % 2L)
        (convertToBinary (n / 2L)) + bit

let convertToPaddedBinary pad n =
...
let folder (map, mask) command =
    match command with
    | Mask m ->
        (map, m)
    | MemoryWrite (cell, value) ->
        (map |> Map.add cell (value, mask), mask)

let applyMask (value, mask:string) =
    let zipper r m s =
        let newChar = if m = 'X' then s else m
        r + string newChar

    convertToPaddedBinary 36 value
    |> Seq.fold2
        zipper
        ""
        mask
...
let result1 input =
    input
    |> Seq.map parse
    |> Seq.fold folder (Map.empty, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
    |> fst
    |> Seq.map (fun x -> applyMask x.Value)
    |> Seq.reduce (+)
