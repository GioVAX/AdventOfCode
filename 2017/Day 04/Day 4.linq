...
//"aa bb cc dd ee", "aa bb cc aa", "aa bb cc dd aaa" 
...
//Step 1
input.Count( s => s.Split(' ').GroupBy( w => w ).All( g => g.Count() == 1 )).Dump();  

//Step 2
input.Count( s => s.Split(' ').Select( w => new string( w.ToCharArray().OrderBy(c => c).ToArray()  )).GroupBy( w => w ).All( g => g.Count() == 1 )).Dump();
